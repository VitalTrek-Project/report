...
| TS03 | Endpoint de gestión de expediciones | Como developer, quiero exponer endpoints para iniciar, consultar y finalizar expediciones para que el frontend pueda gestionar el ciclo de vida de cada recorrido activo. | **Escenario 1: Inicio exitoso**; Given que el request contiene un tourId válido y un token de guía autorizado; When el cliente consume el endpoint POST /api/v1/expeditions; Then el sistema retorna una respuesta con código 201 y los datos de la expedición iniciada; **Escenario 2: Consulta de checkpoints**; Given que existen registros de checkpoints para la expedición solicitada; When el cliente consume el endpoint GET /api/v1/expeditions/{id}/checkpoints; Then el sistema retorna una respuesta con código 200 y la lista de checkpoints con su estado; **Escenario 3: Finalización exitosa**; Given que la expedición se encuentra en estado activo; When el cliente consume el endpoint PUT /api/v1/expeditions/{id}/finish; Then el sistema retorna una respuesta con código 200 confirmando el cierre de la expedición; **Escenario 4: Solicitud sin autorización**; Given que el request no contiene un token de acceso válido; When el cliente consume el endpoint; Then el sistema retorna una respuesta con código 401 indicando que se requiere autenticación | EP07 |
| TS04 | Endpoint de monitoreo de ubicación | Como developer, quiero exponer endpoints que registren y expongan las ubicaciones de los turistas en tiempo real para que el frontend pueda supervisar la seguridad de los participantes durante la expedición. | **Escenario 1: Registro exitoso**; Given que el request contiene coordenadas válidas, el identificador del turista y un token válido; When el cliente consume el endpoint POST /api/v1/locations; Then el sistema retorna una respuesta con código 201 confirmando el registro de ubicación; **Escenario 2: Consulta exitosa**; Given que existen registros de ubicación para la expedición solicitada; When el cliente consume el endpoint GET /api/v1/expeditions/{id}/locations; Then el sistema retorna una respuesta con código 200 y los datos de ubicación de los participantes; **Escenario 3: Sin datos disponibles**; Given que no existen registros de ubicación para la expedición consultada; When el cliente consume el endpoint GET /api/v1/expeditions/{id}/locations; Then el sistema retorna una respuesta con código 200 y una lista vacía; **Escenario 4: Solicitud sin autorización**; Given que el request no contiene un token de acceso válido; When el cliente consume el endpoint; Then el sistema retorna una respuesta con código 401 indicando que se requiere autenticación | EP07 |
| TS05 | Endpoint de gestión de incidentes | Como developer, quiero exponer endpoints que registren y expongan los incidentes reportados durante las expediciones para que el administrador de agencia pueda responder oportunamente. | **Escenario 1: Registro exitoso**; Given que el request contiene información válida del incidente y un token válido; When el cliente consume el endpoint POST /api/v1/incidents; Then el sistema retorna una respuesta con código 201 y los datos del incidente registrado; **Escenario 2: Consulta exitosa**; Given que existen incidentes registrados para la expedición solicitada; When el cliente consume el endpoint GET /api/v1/expeditions/{id}/incidents; Then el sistema retorna una respuesta con código 200 y la lista de incidentes; **Escenario 3: Datos incompletos**; Given que el request omite campos obligatorios del incidente; When el cliente consume el endpoint POST /api/v1/incidents; Then el sistema retorna una respuesta con código 400 indicando los campos requeridos faltantes; **Escenario 4: Solicitud sin autorización**; Given que el request no contiene un token de acceso válido; When el cliente consume el endpoint; Then el sistema retorna una respuesta con código 401 indicando que se requiere autenticación | EP07 |
| TS06 | Endpoint de sincronización offline | Como developer, quiero exponer un endpoint que procese los datos almacenados localmente al recuperar conexión para que no se pierdan registros generados durante la expedición. | **Escenario 1: Sincronización exitosa**; Given que el request contiene datos válidos almacenados localmente; When el cliente consume el endpoint POST /api/v1/sync; Then el sistema procesa los datos correctamente y retorna una respuesta con código 200 confirmando la sincronización; **Escenario 2: Datos duplicados**; Given que el request contiene registros previamente sincronizados; When el cliente consume el endpoint POST /api/v1/sync; Then el sistema ignora los duplicados y retorna una respuesta con código 200 indicando los procesados y los omitidos; **Escenario 3: Error durante la sincronización**; Given que ocurre un fallo interno al procesar los datos; When el cliente consume el endpoint POST /api/v1/sync; Then el sistema retorna una respuesta con código 500 indicando que la sincronización no pudo completarse | EP07 |
| TS07 | Endpoint de gestión de alertas | Como developer, quiero exponer endpoints que generen y expongan alertas ante anomalías detectadas para que el administrador de agencia y el guía de tour puedan responder oportunamente. | **Escenario 1: Alerta generada exitosamente**; Given que el sistema detecta una condición que supera los umbrales configurados; When el endpoint procesa los datos recibidos; Then el sistema registra la alerta y retorna una respuesta con código 201 con los datos de la alerta generada; **Escenario 2: Consulta de alertas activas**; Given que existen alertas activas para la expedición solicitada; When el cliente consume el endpoint GET /api/v1/expeditions/{id}/alerts; Then el sistema retorna una respuesta con código 200 y la lista de alertas activas; **Escenario 3: Parámetros inválidos**; Given que el request contiene datos con formato incorrecto; When el cliente consume el endpoint; Then el sistema retorna una respuesta con código 400 indicando los parámetros inválidos; **Escenario 4: Solicitud sin autorización**; Given que el request no contiene un token de acceso válido; When el cliente consume el endpoint; Then el sistema retorna una respuesta con código 401 indicando que se requiere autenticación | EP07 |
| TS08 | Endpoint de ingesta de telemetría IoT | Como developer, quiero exponer un endpoint que reciba y procese la telemetría enviada por los dispositivos wearables para que la información biométrica esté disponible para el monitoreo en tiempo real. | **Escenario 1: Ingesta exitosa**; Given que el request contiene telemetría válida proveniente de un dispositivo registrado; When el cliente consume el endpoint POST /api/v1/telemetry; Then el sistema registra la telemetría asociada al turista y retorna una respuesta con código 201; **Escenario 2: Dispositivo no registrado**; Given que el identificador del dispositivo no está registrado en el sistema; When el cliente consume el endpoint POST /api/v1/telemetry; Then el sistema retorna una respuesta con código 403 indicando que el dispositivo no está autorizado; **Escenario 3: Telemetría con formato inválido**; Given que el request contiene datos con formato incorrecto; When el cliente consume el endpoint POST /api/v1/telemetry; Then el sistema retorna una respuesta con código 400 indicando los parámetros inválidos; **Escenario 4: Solicitud sin credenciales del dispositivo**; Given que el request no contiene las credenciales del dispositivo; When el cliente consume el endpoint; Then el sistema retorna una respuesta con código 401 indicando que se requiere autenticación del dispositivo | EP07 |
| TS09 | Endpoint de perfil y notificaciones | Como developer, quiero exponer endpoints para gestionar el perfil del usuario y sus preferencias de notificaciones para que el frontend pueda personalizar la experiencia de cada usuario. | **Escenario 1: Consulta de perfil exitosa**; Given que el request contiene un token válido del usuario autenticado; When el cliente consume el endpoint GET /api/v1/profile; Then el sistema retorna una respuesta con código 200 y los datos del perfil del usuario; **Escenario 2: Actualización de preferencias exitosa**; Given que el request contiene preferencias válidas y un token válido; When el cliente consume el endpoint PUT /api/v1/profile/preferences; Then el sistema retorna una respuesta con código 200 confirmando la actualización; **Escenario 3: Consulta de notificaciones**; Given que existen notificaciones registradas para el usuario autenticado; When el cliente consume el endpoint GET /api/v1/notifications; Then el sistema retorna una respuesta con código 200 y la lista de notificaciones; **Escenario 4: Solicitud sin autorización**; Given que el request no contiene un token de acceso válido; When el cliente consume el endpoint; Then el sistema retorna una respuesta con código 401 indicando que se requiere autenticación | EP07 |
| TS10 | Endpoint de exportación de reportes | Como developer, quiero exponer un endpoint que genere reportes exportables de las expediciones para que el administrador de agencia pueda analizar la información operativa. | **Escenario 1: Exportación exitosa**; Given que existen datos disponibles para la expedición solicitada; When el cliente consume el endpoint GET /api/v1/expeditions/{id}/reports/export; Then el sistema retorna una respuesta con código 200 y el archivo generado en el formato solicitado; **Escenario 2: Sin datos para exportar**; Given que no existen datos registrados para la expedición; When el cliente consume el endpoint de exportación; Then el sistema retorna una respuesta con código 404 indicando que no hay información disponible; **Escenario 3: Formato no soportado**; Given que el request especifica un formato de exportación no disponible; When el cliente consume el endpoint; Then el sistema retorna una respuesta con código 400 indicando los formatos soportados; **Escenario 4: Solicitud sin autorización**; Given que el request no contiene un token de acceso válido; When el cliente consume el endpoint; Then el sistema retorna una respuesta con código 401 indicando que se requiere autenticación | EP07 |
### 3.2. Impact Mapping
...
; **Como** gerente de operaciones de una agencia de turismo de aventura,
; **quiero** visualizar en tiempo real la ubicación y estado de mis grupos en ruta,
; **para** reaccionar rápidamente ante incidentes y garantizar la seguridad de los turistas incluso en zonas remotas.
; 
; 
**User Goal 2:** Gestión de incidentes
; **Como** gerente de operaciones,
; **quiero** recibir alertas automáticas cuando ocurra una anomalía o pérdida de conectividad,
; **para** tomar decisiones preventivas y coordinar una respuesta inmediata ante emergencias.
; 
; 
**User Goal 3:** Supervisión centralizada
; **Como** responsable de operaciones turísticas,
; **quiero** supervisar múltiples tours desde un dashboard centralizado,
; **para** reducir la dependencia de reportes manuales y mejorar la eficiencia operativa de la agencia.
; 
; 
**User Goal 4:** Registro y seguimiento de tours
; **Como** gerente de una agencia de aventura,
; **quiero** registrar y monitorear el progreso de cada tour,
; **para** mantener un historial organizado de recorridos, incidentes y desempeño operativo.
; 
; 
**User Goal 5:** Seguridad y reputación de la agencia
; **Como** gerente de operaciones,
; **quiero** asegurar que mis turistas se sientan protegidos durante todo el recorrido,
; **para** fortalecer la reputación de la agencia y ofrecer experiencias seguras y confiables.
; 
---
**User Persona: Marco Rodriguez - Arquitecto Senior en Estudio Independiente**
**User Goal 1:** Seguridad durante la aventura
; **Como** turista,
; **quiero** compartir mi ubicación y mantenerme conectado durante rutas en zonas remotas,
; **para** sentirme seguro y dar tranquilidad a mi familia mientras exploro el Perú.
; 
; 
**User Goal 2:** Comunicación con familiares
; **Como** viajero internacional,
; **quiero** mantener informados a mis familiares sobre mi estado y ubicación,
; **para** reducir su preocupación durante mis recorridos de aventura.
; 
; 
**User Goal 3:** Documentación de la experiencia
; **Como** entusiasta del trekking y la fotografía,
; **quiero** registrar fotos, recorridos y datos de mis rutas,
; **para** conservar recuerdos de calidad y compartir mi experiencia con mi comunidad.
; 
; 
**User Goal 4:** Información contextual del recorrido
; **Como** viajero interesado en la cultura y naturaleza,
; **quiero** acceder a información histórica, geográfica y cultural de las rutas,
; **para** comprender mejor los lugares que visito y enriquecer mi experiencia de viaje.
; 
; 
**User Goal 5:** Planificación segura del viaje
; **Como** turista que organiza sus viajes con anticipación,
; **quiero** contar con información clara sobre rutas, riesgos y recomendaciones,
; **para** tomar decisiones seguras y disfrutar una experiencia bien organizada.
; 
### 4.6. Domain-Driven Software Architecture
